...
# ── PAGE ROUTING ─────────────────────────────────────────────
# Streamlit's modern multipage API requires every page to be
# registered with st.Page() and passed to st.navigation().
#
# The navigation UI is hidden because FinSight uses its own
# custom landing-page buttons via st.page_link().

def render_home():
    """Render the FinSight AI landing page."""
    render_nav()
    render_hero()
    render_path_cards()
    render_how_it_works()
...
stock_page = st.Page(
    ...
    icon="📈",
    url_path="stock-intelligence",
)

mf_page = st.Page(
    "pages/mf_app.py",
    title="MF Recommendation",
    icon="🤖",
    url_path="mf-recommendation",
)

# Register ALL pages before running the selected page.
# position="hidden" keeps Streamlit's default navigation menu
# out of the UI; the landing page provides custom navigation.
pg = st.navigation(
    [home_page, stock_page, mf_page],
    position="hidden",
)
